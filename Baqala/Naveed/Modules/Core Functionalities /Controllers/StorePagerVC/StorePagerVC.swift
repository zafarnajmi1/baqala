//
//  StorePagerViewController.swift
//  Baqala
//
//  Created by apple on 12/7/18.
//  Copyright © 2018 My Technology. All rights reserved.
//

import UIKit
import THSegmentedPager
import Segmentio
import NVActivityIndicatorView
import MIBadgeButton_Swift
import HMSegmentedControl

class StorePagerVC: THSegmentedPager, THSegmentedPageViewControllerDelegate {

    @IBOutlet weak var mySegment: Segmentio!
    
    var storeId = ""
    let badgeBtn = MIBadgeButton(type: .custom)
    
    
    override func viewDidLoad() {
        super.viewDidLoad()

       
        self.pageControl.sectionTitles = ["1", "2"]
        self.configSegments()
        self.loadSegmentList()
        self.setBackButton()
        self.setBarBtnsByAccountType(cartBadgeBtn: badgeBtn)
    }
    
   
    
    
    override func viewDidAppear(_ animated: Bool) {
        //show cart number if value exist
        self.refresh(cartBadgeBtn: badgeBtn)
    }
 

}


extension StorePagerVC {
    
    func configSegments() {
        
        setupSegmentControllers()
        
        self.pageControl.segmentWidthStyle = .fixed
        self.pageControl.segmentEdgeInset = .init(top: 0, left: 20, bottom: 0, right: 20)
        self.pageControl.backgroundColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 0)
        self.pageControl.borderType = HMSegmentedControlBorderType(rawValue: 0)
        
        self.pageControl.selectionIndicatorHeight = 5
        self.pageControl.isVerticalDividerEnabled = false
        
        self.pageControl.selectionIndicatorColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 0)
        self.pageControl.selectionIndicatorBoxColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 0)
        self.pageControl.titleTextAttributes = [
            NSAttributedString.Key.font: UIFont.init(name: "Roboto-Medium", size: 17)!,
            NSAttributedString.Key.foregroundColor:#colorLiteral(red: 1, green: 1, blue: 1, alpha: 0)
        ]
        self.pageControl.selectedTitleTextAttributes = [
            NSAttributedString.Key.font: UIFont.init(name: "Roboto-Medium", size: 17)!,
            NSAttributedString.Key.foregroundColor: #colorLiteral(red: 1, green: 1, blue: 1, alpha: 0)
        ]
       
      
        
    }
    
    fileprivate func setupSegmentControllers() {
     
        let list = ["Store Description".localized, "Store Products".localized]
        let pagesId = ["StoreDetailVC", "ProductListVC"]
        
        self.setupPagesFromStoryboard(withIdentifiers: pagesId)
        
        let storeDetailVc = self.pages[0] as! StoreDetailVC
        storeDetailVc.title = list[0].capitalized
        storeDetailVc.storeId = self.storeId
        storeDetailVc.delegate = self
        
        
        let productVc = self.pages[1] as! ProductListVC
        productVc.title = list[1].capitalized
        productVc.storeId = self.storeId
        productVc.delegate = self
        
      
        
        
        pageControl.indexChangeBlock = { index in
            print("Page selected \(index)")
            self.mySegment.selectedSegmentioIndex = index
        }
     
       
    
    }

}


//MARK:- base configuration ( on load View)
extension StorePagerVC{
    
    fileprivate func loadSegmentList() {
        var content = [SegmentioItem]()
        let list = ["Store Description".localized, "Store Products".localized]
        for value in list {
            let Item = SegmentioItem(title: value, image: nil)
            content.append(Item)
        }
        self.mySegment.configSementio(content)
        self.segmentDidTapped()
    }
    
    
}

//MARK:- actions
extension StorePagerVC {
    
    fileprivate func segmentDidTapped() {
        
        self.mySegment.selectedSegmentioIndex = 0
        self.mySegment.valueDidChange = { segmentio, segmentIndex in
            print("Selected item: ", segmentIndex)
        }
    }
}


//MARK:- Delegates

extension StorePagerVC: ProductListVCDelegate{
    func cartBadgeNumberUpdated(value: Int) {
        self.badgeBtn.showBadge()
        self.badgeBtn.badgeString = "\(value)"
    }
    
    
}

extension StorePagerVC : StoreDetailVCDelegate {
   
    func willApper(bool: Bool) {
        if bool == true {
            self.mySegment.selectedSegmentioIndex = 0
        } else {
              self.mySegment.selectedSegmentioIndex = 1
        }
    }
   
}
